...
function readJson(file,label){
  ...
  try{return {parsed:JSON.parse(bytes.toString('utf8')),sha256:sha(bytes)};}
...
try{
  const captureArtifacts=fs.readdirSync(captureDir).filter(x=>x.endsWith('.json')).sort().map(name=>{
    const loaded=readJson(path.join(captureDir,name),`capture ${name}`);
  ...
  const integrity=readJson(integrityPath,'integrity ledger');
  const lineage=readJson(lineagePath,'lineage manifest');
  const result=runP25DataDrivenExitMultisession({historyPack:history.parsed,captureArtifacts,sessionIntegrityLedger:integrity.parsed,lineageManifest:lineage.parsed,sessionDates:sessionDate?[sessionDate]:null});
