...
function norm(rows=[]){return rows.map(r=>({timestamp:new Date(r.timestamp).toISOString(),open:+r.open,high:+r.high,low:+r.low,close:+r.close,volume:+(r.volume??0)})).filter(r=>[r.open,r.high,r.low,r.close].every(Number.isFinite)).sort((a,b)=>a.timestamp.localeCompare(b.timestamp));}
...
function atr(ctx){if(ctx.length<2)return null;const x=ctx.map((b,i)=>Math.max(b.high-b.low,Math.abs(b.high-(i?ctx[i-1].close:b.open)),Math.abs(b.low-(i?ctx[i-1].close:b.open))));return x.slice(-8).reduce((s,v)=>s+v,0)/Math.min(8,x.length);}

function evExit({entryPrice,direction,setup,sessionDate,path,pool,calibrationRows}){
  const sign=direction==='UP'?1:-1,a=atr(path.slice(0,1))??entryPrice*0.005,hard=sign===1?entryPrice-1.5*a:entryPrice+1.5*a,obs=[],decisions=[],scored=[];
  for(let i=0;i<path.length;i++){
    const b=path[i],hit=sign===1?b.low<=hard:b.high>=hard;
    if(hit){const px=sign===1?(b.open<=hard?b.open:hard):(b.open>=hard?b.open:hard);obs.push(b);return finish(px,'ENTRY_ATR_HARD_STOP',b.timestamp);}
    obs.push(b);
    const state=buildExitState({entryPrice,direction,bars:obs});
    const query={setup,direction,sessionDate,timestamp:b.timestamp,state};
    ...
    const d=decideCalibratedEvExit({raw,calibration:cal});
    ...
    if(raw.ready&&path[j]){
      ...
      if(cal.ready)scored.push({pContinuation:cal.pContinuation,actualContinuation,expectedReturnPct:d.expectedReturnPct,actualReturnPct:actualRet});
    }
    decisions.push({timestamp:b.timestamp,raw,calibration:cal,...d});
    if(d.ready&&d.decision==='EXIT')return finish(b.close,'CALIBRATED_EV_EXIT',b.timestamp);
  }
  return finish(path.at(-1).close,'SESSION_END',path.at(-1).timestamp);
  function finish(px,reason,ts){const gross=dirRet(entryPrice,px,sign),mfe=Math.max(0,...obs.map(b=>dirRet(entryPrice,sign===1?b.high:b.low,sign))),mae=Math.min(0,...obs.map(b=>dirRet(entryPrice,sign===1?b.low:b.high,sign)));return{exitPrice:px,exitReason:reason,exitTimestamp:ts,barsHeld:obs.length,grossReturnPct:gross,netReturnPct:gross-0.05,mfePct:mfe,maePct:mae,profitGivebackPctPoints:Math.max(0,mfe-gross),captureRatio:mfe>0?gross/mfe:null,decisions,calibrationRows:scored};}
}
...
const pairs=[],onlineCalibrationRows=[];for(const s of symbols){let activeUntil=null;for(const c of candidates.filter(x=>x.symbol===s).sort((a,b)=>a.entryTimestamp.localeCompare(b.entryTimestamp))){if(activeUntil&&c.entryTimestamp<=activeUntil)continue;const base=simulateFrozenRatchetExit({entryPrice:c.entryPrice,signalDirection:c.direction==='UP'?'LONG':'SHORT',contextBars:c.context,futureBars:c.path,frozenEntry:true,sessionDate:c.sessionDate});if(!base)continue;activeUntil=base.outcomeAt;const treatment=evExit({...c,pool,calibrationRows:onlineCalibrationRows});const meta={symbol:s,setup:c.setup,direction:c.direction,sessionDate:c.sessionDate,entryTimestamp:c.entryTimestamp,entryPrice:c.entryPrice};const baseline={...meta,exitTimestamp:base.outcomeAt,exitPrice:base.exitPrice,exitReason:base.exitReason,barsHeld:base.barsHeld,grossReturnPct:base.grossReturnPct,netReturnPct:base.netReturnPct,mfePct:base.mfePct,maePct:base.maePct,profitGivebackPctPoints:base.profitGivebackPctPoints,captureRatio:base.captureRatio};pairs.push({baseline,calibratedEv:{...meta,...treatment}});}}
if(!pairs.length)throw new Error('no paired P23.19 trades');
const baselineRows=pairs.map(x=>x.baseline),evRows=pairs.map(x=>x.calibratedEv),b=summarizeEconomicTrades(baselineRows),e=summarizeEconomicTrades(evRows),calRows=evRows.flatMap(x=>x.calibrationRows??[]);
const result={phase:'57.p23.19-calibrated-ev-exit',status:'CALIBRATED_EV_EXIT_REMEASUREMENT_COMPLETE',symbols,symbolCount:symbols.length,policy:P23_19_POLICY,baseline:b,calibratedEv:e,delta:{net:e.averageNetReturnPct-b.averageNetReturnPct,pf:e.profitFactor-b.profitFactor,giveback:e.averageGivebackPctPoints-b.averageGivebackPctPoints,barsHeld:e.averageBarsHeld-b.averageBarsHeld,maxDD:e.maxSequentialDrawdownPctPoints-b.maxSequentialDrawdownPctPoints},calibration:summarizeCalibration(calRows),exitReasons:Object.fromEntries([...new Set(evRows.map(x=>x.exitReason))].map(k=>[k,evRows.filter(x=>x.exitReason===k).length])),pairs,methodology:{strongEntryFrozenQ4:true,exactSameEntriesPaired:true,baselineOccupancyFreezesEntrySet:true,priorSessionAnalogOnly:true,priorSessionCalibrationOnly:true,fullyRealizedHistoricalLabelsOnly:true,noThresholdSweep:true,noEntryRetuning:true,noSymbolFiltering:true,sameSessionOnly:true,developmentRemeasurement:true,freshHoldoutConsumed:false},edgeClaimAllowed:false,recommendationAllowed:false,transmitted:false,...PHASE57_P23_8D_SAFETY};
...
fs.mkdirSync('artifacts',{recursive:true});fs.writeFileSync('artifacts/phase57-p23-19-calibrated-ev-exit.json',JSON.stringify(result,null,2));console.log(JSON.stringify({status:result.status,baseline:result.baseline,calibratedEv:result.calibratedEv,delta:result.delta,calibration:result.calibration,exitReasons:result.exitReasons},null,2));
