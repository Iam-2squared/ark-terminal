...
import {
  SAFETY, buildEntryTimeOpportunity, groupOpportunitySets, allocateV3, summarizePhaseA,
...
function event(symbol, score, rank=1) {
  return {
...
test('builder accepts only first ENTER above frozen threshold and no outcomes', () => {
  const row=buildEntryTimeOpportunity(event('1111.T',0.71),{recentCloses:[100,101,100.5,102]});
  ...
  assert.throws(()=>buildEntryTimeOpportunity({...event('1111.T',0.71),futureReturn:1}),/OUTCOME_FIELD_FORBIDDEN/);
  assert.throws(()=>buildEntryTimeOpportunity(event('1111.T',0.60)),/ENTRY_THRESHOLD_MISMATCH/);
});

test('equal and normalized rank policies are deterministic', () => {
  const ops=[
    buildEntryTimeOpportunity(event('1111.T',0.90,1)),
    buildEntryTimeOpportunity(event('2222.T',0.80,2)),
    buildEntryTimeOpportunity(event('3333.T',0.70,3)),
  ];
...
test('risk-only policy gives lower weight to higher recent volatility', () => {
  const a=buildEntryTimeOpportunity(event('1111.T',0.80),{recentCloses:[100,100.1,100.2,100.3]});
  const b=buildEntryTimeOpportunity(event('2222.T',0.79),{recentCloses:[100,103,97,104]});
...
test('phase A summary exposes distribution only, not realized performance', () => {
  const set=groupOpportunitySets([
    buildEntryTimeOpportunity(event('1111.T',0.90)),
    buildEntryTimeOpportunity(event('2222.T',0.80)),
  ]);
