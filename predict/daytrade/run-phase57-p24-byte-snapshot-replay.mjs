...
let replayRequests=0,aliasRequests=0;
globalThis.fetch=async(url)=>{
  replayRequests++;
  const key=String(url),resolved=snapshot.requestAliases?.[key]||key;
  if(resolved!==key)aliasRequests++;
  const r=snapshot.responses[resolved];
...
const attestation={phase:'57.p24.9-replay',snapshotPath,snapshotPhase:snapshot.phase,snapshotSha256:crypto.createHash('sha256').update(bytes).digest('hex'),responseCount:Object.keys(snapshot.responses).length,replayRequests,aliasRequests,canonicalWindowDays:snapshot.canonicalWindowDays??null,effectiveStartIso:snapshot.effectiveStartIso??null,dataEndIso:snapshot.dataEndIso??null,networkAccessAllowed:false,freshHoldoutConsumed:false,safety:{executionAllowed:false,brokerWriteAllowed:false,excelOrderWriteAllowed:false,rssOrderFunctionAllowed:false,liveTradingAllowed:false,paperTradingAllowed:false,automaticPromotionAllowed:false,productionUpdateAllowed:false,transmitted:false}};
