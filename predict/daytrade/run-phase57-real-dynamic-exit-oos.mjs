...
if (replay.reconciliation && !replay.reconciliation.matches) {
  throw new Error(`P23.3 entry signal replay mismatch: ${JSON.stringify(replay.reconciliation)}`);
}
...
const dynamicExit = evaluateRealDynamicExitOos(exitRows, {
...
const summary = {
  phase: '57.p23.3-real',
  status: 'REAL_5M_CHART_AWARE_DYNAMIC_EXIT_OOS_MEASURED',
  scope,
  ...
  signalReplay: {
    ...
    missingSourceRows,
    missingSessionPaths,
    dynamicExitResearchRows: exitRows.length,
  },
  dynamicExit,
  interpretationRules: {
    primaryObjective: 'NET_EXPECTANCY_AFTER_EXPLICIT_COST',
    fixedElapsedTimeIsMatchedBaselineOnly: true,
    healthyTrendCanRunUntilChartExitOrSessionEnd: true,
    dynamicConfigSelectedFromTheseOuterResults: false,
    postSelectExitVariantFromTheseOuterResults: false,
    edgeClaimFromThisSingleRecentWindowAllowed: false,
  },
  limitations: [
    'Historical order-book/tick-flow microstructure is unavailable in Yahoo 5m OHLCV and is excluded from this remeasurement.',
    ...
    'Dynamic chart-management parameters are frozen before this run and are not selected from these outer outcomes.',
    'Matched fixed baseline uses the prior adaptive entry signal horizon on the exact same signal row.',
    'The full remaining same-session path is supplied; elapsed time is not the primary exit rule.',
    'Max sequential signal drawdown is a signal-order diagnostic, not a capital portfolio drawdown because overlapping signals can exist.',
...
const output = `artifacts/phase57-real-dynamic-exit-oos-${scope}.json`;
fs.writeFileSync(output, JSON.stringify(summary, null, 2));
console.log('PHASE57_P23_3_REAL_DYNAMIC_EXIT_JSON_START');
console.log(JSON.stringify(summary));
console.log('PHASE57_P23_3_REAL_DYNAMIC_EXIT_JSON_END');
