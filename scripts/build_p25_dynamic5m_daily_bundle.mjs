...
for(const file of walk(inputDir)){
  ...
  const selected=x.selected.map(r=>({symbol:String(r?.symbol??'').trim().toUpperCase(),sector:String(r?.sector??'UNKNOWN'),opportunityScore:Number(r?.opportunityScore),turnoverYen:Number(r?.turnoverYen)})).filter(r=>r.symbol);
