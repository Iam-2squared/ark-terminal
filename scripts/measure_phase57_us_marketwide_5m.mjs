...
if(envelope?.plan==='FREE'&&envelope?.freshRealtime!==true)throw new Error('US Fresh blocked: free provider data is not real-time; preserve as diagnostic/EOD only');
...
const payload={schemaVersion:2,phase:'57.us-cross-market.marketwide-5m',status:'US_MARKETWIDE_5M_MEASURED',sessionDate:u.sessionDate,observedAt:new Date().toISOString(),inputSymbols:valid.length,selectedSymbols:selected.length,selectedV2Symbols:selectedV2.length,selected,selectedV2,provider:envelope?{name:envelope.provider,plan:envelope.plan,feedClassification:envelope.feedClassification,sourceObservedAt:envelope.observedAt,rawDigest:envelope.rawDigest}:null,classification:{crossMarketProspective:true,jpxOosSubstitute:false,formalOos:false,promotionEligible:false},methodology:{universeFrozenBeforeOpen:true,pointInTimeOnly:true,futureOutcomeUsed:false,missingSymbolsNeverFabricated:true},policySha256:PHASE57_US_CROSS_MARKET_POLICY_SHA256,safety:PHASE57_US_CROSS_MARKET_SAFETY};
