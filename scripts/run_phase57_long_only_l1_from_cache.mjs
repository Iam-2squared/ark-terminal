...
for(const sessionDate of approvedSessions){
  ...
  const rawTimes=minuteRows.map(row=>String(row?.Time??row?.time??'')).filter(Boolean).sort(),symbolsWithMinute=new Set(minuteRows.map(codeOf).filter(Boolean)),eligibleSymbols=new Set(dailyRows.map(row=>row.symbol)),eligibleSymbolsWithMinute=[...eligibleSymbols].filter(symbol=>symbolsWithMinute.has(symbol)).length;
  sessionAudits.push(Object.freeze({sessionDate,eligibleDailySymbols:dailyRows.length,rawMinuteSymbols:symbolsWithMinute.size,eligibleSymbolsWithMinute,minuteCoverageVsEligiblePct:dailyRows.length?Number((100*eligibleSymbolsWithMinute/dailyRows.length).toFixed(4)):null,firstRawTime:rawTimes[0]??null,lastRawTime:rawTimes.at(-1)??null,pageCount:minuteManifest.pageCount,rawRows:minuteRows.length,...intraday.audit,l1FeatureRows:built.featureRows.length,l1LabelRows:built.evaluatorOnlyLabels.length,nonUnitAdjustmentScaleRows:built.audit.nonUnitAdjustmentScaleRows,minutePriceScale:built.audit.minutePriceScale,exclusions:built.audit.exclusions}));
