...
test('J-Quants one-minute rows aggregate to causal sparse five-minute bars',()=>{
  ...
  ],{sourceMinuteTimestampMeaning:'BAR_OPEN'});
...
test('timestamp meaning must be verified before aggregation',()=>{
  assert.throws(()=>aggregateJquantsMinutesToFiveMinuteBars([row('09:00')]),/explicitly verified/);
});
...
test('conflicting duplicates and out-of-session rows fail closed',()=>{
  ...
  assert.throws(()=>normalizeJquantsMinuteRows([row('11:45')]),/outside the regular TSE session/);
});
