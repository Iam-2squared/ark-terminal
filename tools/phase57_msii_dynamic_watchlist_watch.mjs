...
function rawMeta(file){const payload=readJson(file);const observedAt=iso(payload?.meta?.observedAt,"raw observedAt");return {file,payload,observedAt,bucketAt:new Date(Math.floor(Date.parse(observedAt)/FIVE_MINUTES_MS)*FIVE_MINUTES_MS).toISOString()};}
...
function expectedBuckets(sessionDate){const out=[];for(const [start,end] of [["09:05","11:30"],["12:35","15:30"]]){let at=Date.parse(`${sessionDate}T${start}:00+09:00`),last=Date.parse(`${sessionDate}T${end}:00+09:00`);while(at<=last){out.push(new Date(at).toISOString());at+=FIVE_MINUTES_MS;}}return out;}
...
function loadState(file,sessionDate){if(!fs.existsSync(file))return {schemaVersion:2,sessionDate,lastObservedAt:null,lastBucketAt:null,processedBucketCount:0,priorV2Selections:[],recentV1Selections:[],processedRaw:[],safety:SAFETY};const state=readJson(file);if(state.sessionDate!==sessionDate)throw new Error("dynamic watchlist state sessionDate mismatch");return state;}
...
export function processAvailableRawSnapshots({rawDirectory,sessionDate,state,slotCount=80,retentionPoints=3,laneMStateFile=null}={}){
  assertSafety();
  const files=listRaw(rawDirectory).map(rawMeta).sort((a,b)=>Date.parse(a.observedAt)-Date.parse(b.observedAt)||a.file.localeCompare(b.file));
  const schedule=expectedBuckets(sessionDate);
  let next={...state,priorV2Selections:[...(state?.priorV2Selections??[])],recentV1Selections:[...(state?.recentV1Selections??[])],processedRaw:[...(state?.processedRaw??[])],processedBucketCount:Number(state?.processedBucketCount??0),safety:SAFETY};
  const outputs=[];
  for(const item of files){
    if(next.processedRaw.includes(path.basename(item.file)))continue;
    const expected=schedule[next.processedBucketCount]??null;
    if(!expected){next.processedRaw.push(path.basename(item.file));continue;}
    if(item.bucketAt!==expected){
      const watchlist={schemaVersion:1,status:item.bucketAt<expected?"BLOCKED_DYNAMIC_RAW_DUPLICATE_OR_STALE_BUCKET":"BLOCKED_DYNAMIC_RAW_CADENCE_GAP",complete:false,observedAt:item.observedAt,bucketAt:item.bucketAt,expectedBucketAt:expected,processedBucketCount:next.processedBucketCount,assignedSymbols:[],futureOutcomeUsed:false,safety:SAFETY};
      outputs.push({status:"BLOCKED",file:item.file,watchlist});break;
    }
    ...
    next={...next,lastObservedAt:item.observedAt,lastBucketAt:item.bucketAt,processedBucketCount:next.processedBucketCount+1,priorV2Selections:watchlist.v2PriorSelectionsNext,recentV1Selections:watchlist.recentV1SelectionsNext,processedRaw:[...next.processedRaw,path.basename(item.file)],safety:SAFETY};
    outputs.push({status:"READY",file:item.file,watchlist:{...watchlist,bucketAt:item.bucketAt,expectedBucketAt:expected}});
  }
    ...
    if(processed.outputs.length){
      ...
      for(const item of processed.outputs){
        if(item.status==="READY"){
          ...
          process.stdout.write(`${JSON.stringify({status:item.watchlist.status,observedAt:item.watchlist.observedAt,bucketAt:item.watchlist.bucketAt,assignedSymbolCount:item.watchlist.assignedSymbolCount,pinnedSymbolCount:item.watchlist.pinnedSymbols.length,retainedSymbolCount:item.watchlist.retainedSymbols.length,safety:SAFETY})}\n`);
        }else{
          process.stderr.write(`${JSON.stringify({status:item.watchlist.status,observedAt:item.watchlist.observedAt,bucketAt:item.watchlist.bucketAt??null,expectedBucketAt:item.watchlist.expectedBucketAt??null,requiredSymbolCount:item.watchlist.requiredSymbolCount??null,slotCount:item.watchlist.slotCount??slotCount,overflowRequiredSymbols:item.watchlist.overflowRequiredSymbols??[],safety:SAFETY})}\n`);
          return 2;
        }
      }
    }
...
export { expectedBuckets as expectedPhase57MsiiDynamicBuckets };
