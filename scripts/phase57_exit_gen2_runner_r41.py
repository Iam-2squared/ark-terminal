"""R41 one-shot finite classifiers and immutable-prediction causal replay.
...
two classifier scores, freshness, prior counter, candidate and decision NOW.
"""
...
from scripts import phase57_exit_gen2_data_r41 as data_builder
from scripts import phase57_exit_gen2_preflight_r41 as preflight
...
def support_slices(data, protocol: dict) -> tuple[list[tuple], list[dict]]:
    ...
    require(data.targets.shape == (len(data.sessions), 2), "TARGET_SHAPE")
    ...
    for fold in protocol["split"]["folds"]:
        ...
        for arm_code, arm in enumerate(protocol["entryArms"]):
            ...
            for head, name in enumerate(HEADS):
                ...
                train = base[np.isfinite(data.targets[base, head])]
    ...
    require(len(slices) == protocol["execution"]["folds"] * 2 * 2, "SUPPORT_SLICE_COUNT")
    return slices, rows
...
def fit_predictions(data, out: Path, protocol: dict) -> tuple[dict, dict]:
    ...
    require(len(specs) == 4 and len({s["specId"] for s in specs}) == 4, "FOUR_UNIQUE_SPECS_REQUIRED")
    require(protocol["execution"]["expectedModelFitCount"] == 64, "FIT_BUDGET_NOT_64")
    ...
    write_json(out / "fit-start.json", {"status": "FITTING_AUTHORIZED_SUPPORT_COMPLETE",
                "expectedModelFits": 64, "candidateCount": len(protocol["candidates"]), "safety": SAFETY})
    model_dir = out / "models"
    model_dir.mkdir(parents=True, exist_ok=False)
    predictions = {s["specId"]: np.full((len(data.sessions), 2), np.nan, dtype=np.float32) for s in specs}
    model_rows = []
    with (out / "fit-progress.jsonl").open("xb") as journal:
        for fold, arm, head, train, score in slices:
            ...
            for spec in specs:
                ...
                if not model_rows:
                    print("FITTING_STARTED " + r36.canonical({"expectedModelFits": 64,
    ...
    require(len(model_rows) == 64, "MODEL_FIT_COUNT_NOT_64")
    ...
    receipt = {"schemaVersion": "phase57-exit-gen2-fit-r41-v1", "modelFits": len(model_rows),
               "models": model_rows, "predictionSha256": sha(pred_path), "heads": list(HEADS),
               "predictionSpecs": 4, "calibratedProbabilities": False, "safety": SAFETY,
...
def replay_candidate(data, scores: np.ndarray, candidate: dict, protocol: dict,
    ...
    require(scores.shape == (len(data.sessions), 2), "REPLAY_SCORE_SHAPE")
    ...
    for entry_code in sorted(groups):
        ...
        for idx in sequence:
            ...
            result = ({"action": "FORCE_TERMINAL" if now == 925 else "HOLD", "consecutive": 0}
                      if neutral else runtime.intent(scores[idx].tolist(), bool(data.fresh[idx]),
                                                     count, candidate, now))
...
def replay_ab(data, out: Path, protocol: dict, fit_receipt: dict) -> dict:
    ...
    require(len(candidates) == 16, "CANDIDATE_COUNT_NOT_16")
    manifests = {}
    for label in ("run-a", "run-b"):
        ...
        for candidate in candidates:
            ...
            filename = candidate["candidateId"] + ".jsonl.gz"
            r36.write_jsonl_gz(run_dir / filename, ledger)
            files[filename] = sha(run_dir / filename)
        neutral = replay_candidate(data, np.full((len(data.sessions), 2), np.nan),
                                   candidates[0], protocol, neutral=True)
        filename = "HOLD_TO_TERMINAL_DIAGNOSTIC.jsonl.gz"
        r36.write_jsonl_gz(run_dir / filename, neutral)
        files[filename] = sha(run_dir / filename)
    ...
    receipt = {"runABByteIdentical": True, "ledgerHashes": manifests["run-a"],
               "candidatePoliciesReplayed": 16, "policyReplayPasses": 32,
...
def fit_replay(out: Path, cache: Path, protocol: dict, execution_sha: str,
    ...
    write_json(out / "receipt.json", {
        "schemaVersion": "phase57-exit-gen2-pending-audit-r41-v1",
