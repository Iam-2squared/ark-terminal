...
export function buildFourTrackDurable({measurement,marketwideNdjson,laneC,entryRef,exitRef,selectionRef,laneCRef,sourceRunId}){
  ...
  return {
    ...
    tracks:{
      ...
      dynamic5m:{status:'DURABLE_APPEND_ONLY',ref:selectionRef,inputSymbols:Number(measurement.inputSymbols),selectedCount:Number(measurement.selectedCount??measurement.selectedSymbols?.length??0),measurementSha256:sha(measurement),snapshotStateHash:String(snapshot.stateHash??''),snapshotSha256:sha(rows[0])},
