...
readVerified(releasePath,CONTRACT.baselineReleaseSha256,'MANIFEST');
const gateDir=path.join(base,'predict/research/phase57-entry-baseline-real-gates-2026-09-09');
const gates=read(path.join(gateDir,'gates.json'));
const inventory=JSON.parse(gunzipSync(fs.readFileSync(path.join(gateDir,'archive-inventory.json.gz')))).inventory;
...
const sourceManifest=read(path.join(measurement,'evidence-manifest.json'));
