...
export const P23_49_POLICY=Object.freeze({phase:'57.p23.49',priorSessionsOnly:true,baselineCoveragePreserved:true,consensusGate:true,leaveCurrentSymbolOutEvidence:true,gateUsesCurrentOos:false,thresholdSearchAllowed:false,exitPolicyChangeAllowed:false,entryRetuningAllowed:false,symbolFilteringAllowed:false,freshHoldoutConsumed:false,purpose:'TEST_PRIOR_ONLY_CONSENSUS_GATE_REQUIRING_GLOBAL_AND_LEAVE_SYMBOL_OUT_EVIDENCE'});
...
export function walkForwardConsensusPriorFamilyGatedExact(rows=[],baselineRows=[]){
 const xs=[...rows].sort((a,b)=>a.timestamp.localeCompare(b.timestamp)||a.symbol.localeCompare(b.symbol)),byKey=new Map(xs.map(r=>[key(r),r])),cache=new Map(),out=[];let cdfSelectedCount=0,baselineSelectedCount=0,baselinePreservedCount=0;
 for(const b of baselineRows){const q=byKey.get(key(b));if(!q){out.push({...b,scoreSource:'EXACT_P23_30_BASELINE'});baselinePreservedCount++;continue;}
  const all=xs.filter(r=>r.setup===q.setup&&r.direction===q.direction&&r.sessionDate<q.sessionDate&&String(r.fullyRealizedAt)<String(q.timestamp)),loo=all.filter(r=>r.symbol!==q.symbol);
  const gk=`${q.setup}|${q.direction}|${q.symbol}|${q.sessionDate}`;let g=cache.get(gk);if(!g){g={all:evidence(all),loo:evidence(loo)};cache.set(gk,g);}
  if(g.all.family==='P2344'&&g.loo.family==='P2344'){const m=fitCdfContrastModel(all),riskScore=scoreCdfContrast(q,m);if(Number.isFinite(riskScore)){out.push({...b,riskScore,scoreSource:'CONSENSUS_PRIOR_GATE_P2344',gateAllBaselineAuc:g.all.baselineAuc,gateAllCdfAuc:g.all.cdfAuc,gateLooBaselineAuc:g.loo.baselineAuc,gateLooCdfAuc:g.loo.cdfAuc});cdfSelectedCount++;continue;}}
  const m=fitDynamicModel(all),riskScore=scoreDynamic(q,m);if(Number.isFinite(riskScore)){out.push({...b,riskScore,scoreSource:'CONSENSUS_PRIOR_GATE_P2330'});baselineSelectedCount++;}else{out.push({...b,scoreSource:'EXACT_P23_30_BASELINE'});baselinePreservedCount++;}
 }
