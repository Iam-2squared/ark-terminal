...
export function generateExtendedFeatures(records = []) {
  ...
  for (const [symbol, rows] of bySymbol) {
    ...
    for (let index = 19; index < rows.length; index += 1) {
      ...
      const vwap = volumeSum === 0 ? Number(current.close) : typicalPrice.reduce((sum, value, i) => sum + (value * volumes[i]), 0) / volumeSum;
      ...
      output.push(Object.freeze({
        ...
        vwap20: vwap,
        bollingerZ20: sigma20 === 0 ? 0 : (Number(current.close) - sma20) / sigma20,
...
export function buildDatasetLineage({ datasetVersion, sourceManifestChecksum, featureVersion = "phase46-advanced-v1", rows = [] } = {}) {
