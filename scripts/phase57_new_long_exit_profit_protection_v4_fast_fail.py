"""NEW LONG EXIT Profit Protection v4: after +3, protect +1 completed-CLOSE floor."""
...
CONTRACT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v4-contract-2026-09-18.md';OUT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v4-fast-fail'
def mod(n,p):
...
def sim(rows,fallback):
 ...
 for i,b in enumerate(rows):
  ...
  if b['c']<=1:
   ...
   trans.append([b['slot'],'PLUS1_FLOOR_BREAK',peak,b['c']])
...
def build():
 v0=mod('pp_v0_for_v4',V0);old=v0.sim;v0.sim=sim
 try:ledger,s=v0.build()
 finally:v0.sim=old
 s['status']='NEW_LONG_EXIT_PROFIT_PROTECTION_V4_FAST_FAIL_PASS' if all(s['gates'].values()) else 'NEW_LONG_EXIT_PROFIT_PROTECTION_V4_FAST_FAIL_KILL'
 return ledger,s
def run(outdir):
