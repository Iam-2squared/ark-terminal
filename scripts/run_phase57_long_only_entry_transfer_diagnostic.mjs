...
import {auditEntryFeasibilityEvent} from '../predict/long-only/phase57-long-only-entry-preimplementation-feasibility.js';
...
const l0s=new Map(),allEvents=[],allOpportunities=[],allTicks=[],allFeasibilityEvents=[],sessionAudits=[];
...
for(const sessionDate of frozenSessions){
  ...
  for(const feature of built.featureRows){
    ...
    const candidate={sessionDate,symbol:String(feature.symbol),decisionTimestamp:feature.decisionAtJst,decisionTimeJst:feature.decisionTimeJst,
      decisionPrice:Number(semantics.decisionPrice),decisionPriceAgeMinutes:Number(semantics.referenceAgeMin),
      decisionPriceSource:String(semantics.decisionPriceKind),ridgeScore:Number(scoreL2Candidate(feature,artifact)),selectorFeatureTimestamp:feature.decisionAtJst,
  ...
  const transferEventById=new Map(transfer.events.map(row=>[row.selectorEventId,row])),lineage=new Map();
  for(const selection of selections.sort((left,right)=>left.decisionTimestamp.localeCompare(right.decisionTimestamp)||left.ridgeRank-right.ridgeRank)){
    const state=lineage.get(selection.symbol)??{firstSelectionTimestamp:selection.decisionTimestamp,priorSelectionCount:0};
    const transferEvent=transferEventById.get(selection.selectorEventId);
    allFeasibilityEvents.push(auditEntryFeasibilityEvent({event:{...selection,direction:'LONG'},
      bars:entryBars.get(selection.symbol)??[],minutes:provenance.get(symkey(sessionDate,selection.symbol))??[],
      auctions:entryAuctions.get(selection.symbol)??[],firstSelectionTimestamp:state.firstSelectionTimestamp,
      priorSelectionCount:state.priorSelectionCount,currentAtomicStatus:transferEvent?.directStatus??null,
      currentAtomicReason:transferEvent?.directReason??null}));
    state.priorSelectionCount+=1;lineage.set(selection.symbol,state);
  }
...
allFeasibilityEvents.sort((a,b)=>a.sessionDate.localeCompare(b.sessionDate)||a.decisionTimestamp.localeCompare(b.decisionTimestamp)||a.ridgeRank-b.ridgeRank||a.symbol.localeCompare(b.symbol));
...
if(allFeasibilityEvents.length!==allEvents.length||new Set(allFeasibilityEvents.map(row=>row.selectorEventId)).size!==allEvents.length)throw new Error('FEASIBILITY_EVENT_IDENTITY_MISMATCH');
if(allFeasibilityEvents.some(row=>row.direction!=='LONG'||row.shortScoreEvaluated!==false))throw new Error('FEASIBILITY_LONG_ONLY_CONTRACT_VIOLATION');
...
const eventsPath=path.join(outputDir,'selector-current-entry-events.ndjson'),opportunitiesPath=path.join(outputDir,'first-entry-opportunities.json'),
  ticksPath=path.join(outputDir,'entry-evaluation-ticks.ndjson'),feasibilityPath=path.join(outputDir,'entry-preimplementation-feasibility-events.ndjson');
...
fs.writeFileSync(feasibilityPath,allFeasibilityEvents.map(row=>JSON.stringify(row)).join('\n')+'\n',{flag:'wx',mode:0o600});
const manifest={schemaVersion:1,status:'FROZEN_SELECTOR_CURRENT_ENTRY_TRANSFER_INPUT_READY',contractId:transferContract.contractId,
  ...
  counts:{sessions:76,decisionTimestamps:decisionCount,selectionEvents:allEvents.length,uniqueSymbols:new Set(allEvents.map(row=>row.symbol)).size,
    ...
    entryPreimplementationFeasibilityEvents:allFeasibilityEvents.length,
    ordinal30mLabelableEvents:allFeasibilityEvents.filter(row=>row.label.labelable).length},
  files:[{path:path.basename(eventsPath),sha256:sha(fs.readFileSync(eventsPath)),rows:allEvents.length},
    ...
    {path:path.basename(feasibilityPath),sha256:sha(fs.readFileSync(feasibilityPath)),rows:allFeasibilityEvents.length}],
