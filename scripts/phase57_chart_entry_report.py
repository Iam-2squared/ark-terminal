...
def report(evidence,replay):
 ...
 # Analog uncertainty and explicit missed-winner reasons; reporting only.
 analog_rows=read(m/'analog-ledger.json.gz');fig,axs=plt.subplots(1,2,figsize=(12,4));axs[0].hist([x['distanceMean'] for x in analog_rows if x['distanceMean'] is not None],bins=30);axs[0].set_title('Analog neighbor distance');axs[1].hist([x['uncertainty'] for x in analog_rows if x['uncertainty'] is not None],bins=30);axs[1].set_title('Analog return mean standard error');save(fig,'16-analog-uncertainty')
 missed=read(m/'missed-winners.json');fig,axs=plt.subplots(1,2,figsize=(13,4))
 for ax,level in zip(axs,[3,5]):
  bottom=np.zeros(len(arms))
  for reason in ['CAPTURED','NO_ENTRY','OUTCOME_UNAVAILABLE','UPSIDE_LOST_BEFORE_OR_BY_ENTRY_PRICE']:
   vals=[sum(n for k,n in missed[arm].items() if k.startswith(str(level)+'/') and k.endswith('/'+reason)) for arm in arms];ax.bar(arms,vals,bottom=bottom,label=reason);bottom+=np.array(vals)
  ax.set_title(f'+{level}% winners: fixed denominator');ax.tick_params(axis='x',rotation=45);ax.legend(fontsize=6)
 save(fig,'17-missed-winners')
 ...
 lines += ['', f"Frozen元特徴の再構成: {sum(x.get('frozenFeatureProjectionAvailable',0) for x in inventory)}候補で保存score再現、{sum(x.get('frozenFeatureProjectionUnavailable',0) for x in inventory)}候補でUNAVAILABLE。保存55日側は元のpartition-wide L0履歴と今回の直前1日入力が同一とは限らず、score不一致の特徴はEntryへ渡していない。保存candidate/score/rank/priceは変更なし。日別不一致値はsubstrate/inventory.jsonに保存。この制約を含むEntry比較であり、全Frozen元特徴を完全復元できたとは主張しない。", '', '各Entry後outcomeの可用件数は異なる。比較表は条件付き分布、effects.jsonはpaired entrants、captureは全Selector winner固定分母（未Entry・outcome不明も保持成功にしない）。この3つを併読し、BUY削減だけによるMAE改善を成功としない。','', '## 判定','', '| Arm | Model selected before evaluation | Judgment |','|---|---|---|']
 for k in e.ARMS:lines.append(f"| {k} | {chosen['chosen'][k]} | {dec[k]['status']} |")
 lines += ['', '**WHO正式trait効果はDICTIONARY_ENTRY_VALUE_INCONCLUSIVE。** 数値valueはH/M+PASSのみ、その他traitValue/confidence/temporal/uncertainty/nEff/computedThrough/evidenceClassは別状態として保存。欠測を性格0として扱わない。','', 'Historical Analogはquery前の既にsession終了した初回Opportunity caseのみ。近傍20、最低5、最大直近10000 case。same-session futureはpoolに入らない。後日評価中の過去case追加入力は事前固定したonline retrievalであり、Entryモデル再学習ではない。各queryのneighbors・maxRealizedThroughを保存。','', '成功Gate: +3/+5 capture各90%以上、Immediate比enter数90%以上、paired MAE改善0.1pp以上・session block5 bootstrap95%下限>0・paired entry価格改善>0。CURRENT比較・ablation差はeffects.jsonで個別表示し、他情報源の効果に読み替えない。','', '## 図表','', 'success/failure chart例はEntry価格改善の正負で分類し、固定hash順に抽出。利益の成功例という意味ではない。将来outcomeは別JSONに分離。' ]
