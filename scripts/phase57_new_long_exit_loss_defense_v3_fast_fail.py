"""NEW LONG EXIT Loss Defense v3: completed CLOSE <= -3% tail boundary."""
...
CONTRACT=ROOT/'docs/evidence/phase57-new-long-exit-v3-tail-boundary-contract-2026-09-18.md';OUT=ROOT/'docs/evidence/phase57-new-long-exit-v3-tail-boundary-fast-fail'
def mod(n,p):
...
def sim(rows,fallback,cost=0,maxslot=None):
 ...
 for i,b in enumerate(rows):
  ...
  if b['c']<=-3:
   ...
   trans.append([b['slot'],'CLOSE_TAIL_BOUNDARY_MINUS3',b['c']])
...
def build():
 v0=mod('ld_v0_for_v3',V0);old=v0.sim;v0.sim=sim
 try:own,fixed,s=v0.build()
 finally:v0.sim=old
 s['status']='NEW_LONG_EXIT_LOSS_DEFENSE_V3_FAST_FAIL_PASS' if all(s['gates'].values()) else 'NEW_LONG_EXIT_LOSS_DEFENSE_V3_FAST_FAIL_KILL'
 return own,fixed,s
def run(outdir):
