...
for(let pointIndex=0;pointIndex<measurements.length;pointIndex++){
  for(let symbolIndex=0;symbolIndex<d50.length;symbolIndex++){
    const flatIndex=pointIndex*d50.length+symbolIndex;
    if(flatIndex%shardCount===shardIndex)assignedPairs.push({flatIndex,pointIndex,symbolIndex,symbol:d50[symbolIndex],observedAt:String(measurements[pointIndex].observedAt)});
  }
}
...
for(const pair of assignedPairs){
  ...
  if(!usableSymbolSet.has(symbol))continue;
  const observedMs=Date.parse(observedAt);
  const full=sessionBarsBySymbol[symbol]??[];
  const prefix=full.filter(bar=>Date.parse(bar.timestamp)+5*60_000<=observedMs);
  if(prefix.length<6){out.blockedReason=out.blockedReason??`INSUFFICIENT_CLOSED_PREFIX:${symbol}:${prefix.length}`;continue;}
  ...
  let result;
  try{result=scorePrefix(currentPrefix);}catch(error){out.blockedReason=out.blockedReason??`SCORER_EXCEPTION:${symbol}:${String(error?.message??error)}`;continue;}
  if(!result?.complete||!result?.decision){out.blockedReason=out.blockedReason??`SCORER_NOT_READY:${symbol}:${result?.status??'UNKNOWN'}`;continue;}
  const d=result.decision,context=d.context??{};
  if(d.futureOutcomeUsed!==false||d.frozenByPhase57!==true||d.pointInTimeOnly!==true){out.blockedReason=out.blockedReason??`FROZEN_ENTRY_ATTESTATION_FAILED:${symbol}`;continue;}
  out.scoredCount++;
  const direction=Number(d.direction);
  if(context.signalEligible!==true||![-1,1].includes(direction))continue;
...
fs.mkdirSync(path.dirname(output),{recursive:true});
fs.writeFileSync(output,JSON.stringify(payload,null,2)+'\n');
console.log(JSON.stringify({status:payload.status,shardIndex,assignedPairs:assignedPairs.length,points:points.length,blocked:points.filter(x=>x.blockedReason).length,signals:points.reduce((s,x)=>s+x.signals.length,0)},null,2));
