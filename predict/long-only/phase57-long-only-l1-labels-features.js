...
export function buildCausalL1Features({sessionDate,symbol,decisionTimeJst,bars5m,previousAdjustedClose,segment='UNKNOWN',corporateActionFlag=false}={}){
  ...
  const feature={sessionDate,symbol,segment,decisionTimeJst,decisionAtJst,currentPrice:last.close,currentReturnPct:pct(last.close,previousAdjustedClose),momentum5Pct:momentum5,momentum15Pct:momentum15,momentum30Pct:momentum30,momentumAccelerationPct:momentum5===null||momentum15===null?null:momentum5-momentum15/3,cumulativeVolume:totalVolume,cumulativeTurnover:totalTurnover,vwap,vwapDistancePct:vwap?pct(last.close,vwap):null,sessionHighUpdate:last.high>=Math.max(...bars.map(x=>x.high)),rangeExpansionPct:pct(Math.max(...bars.map(x=>x.high)),Math.min(...bars.map(x=>x.low))),trendEfficiency:path>0?Math.abs(last.close-bars[0].open)/path:0,reversalCount:directionChanges,causalAtr:trueRanges.reduce((a,x)=>a+x,0)/trueRanges.length,observedBars:bars.length,latestAvailableAtJst:last.availableAtJst};
  return Object.freeze(Object.fromEntries(Object.entries(feature).map(([k,v])=>[k,typeof v==='number'?round(v):v])));
}

export function buildEvaluatorOnlyL1Label({sessionDate,symbol,decisionTimeJst,bars5m,terminalAuctions=[],previousAdjustedClose,officialFinalAdjustedClose,decisionTimeAtr,contract=L1_LABEL_CONTRACT}={}){
  ...
  const label={sessionDate,symbol,decisionTimeJst,evaluatorOnly:true,finalReturnPct,winner:finalReturnPct>=contract.winnerThresholdPct,remainingUpsidePct,futureMfePct:remainingUpsidePct,futureMaePct,decisionTimeAtrPct:atrPct,futureMfeAtr:atrPct?remainingUpsidePct/atrPct:null,futureMaeAtr:atrPct?futureMaePct/atrPct:null,continuationToClosePct:pct(finalClose,current),lateDetection:finalReturnPct>=contract.winnerThresholdPct&&remainingUpsidePct<contract.lateDetectionRemainingUpsidePct,futureBarCount:future.length};
...
export function buildL1ResearchRows(args={}){
  ...
  for(const decisionTimeJst of L1_DECISION_TIMES_JST){
    ...
    const label=buildEvaluatorOnlyL1Label({...args,decisionTimeJst,decisionTimeAtr:feature.causalAtr});if(label)labels.push(label);
  }
