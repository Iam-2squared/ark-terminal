...
const outputPath = arg('--output', '/tmp/phase57-exit-v5-gbm-development-validation-evidence.json');
if (!inputDir) {
  console.error('usage: node scripts/reduce_phase57_exit_v5_gbm_validation_shards.mjs --input-dir <dir> [--output <json>]');
  process.exit(2);
}
...
try {
  ...
  const result = reduceExitV5DevelopmentValidationGbmShards(shards);
  ...
  console.error(JSON.stringify({
    status: 'BLOCKED_EXIT_V5_GBM_VALIDATION_REDUCE',
