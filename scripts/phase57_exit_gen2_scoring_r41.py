"""R41 evaluator/selection only, invoked in the NEXT Work after artifact audit.

This module is not imported or called by the finite learning workflow. The
numeric R25/R31 gates are unchanged; the explicit R41 rank-tie rule is new.
"""
...
def select_from_gates(results):
    ...
    frozen.require(len(results) == len(expected)
                   and {r['candidateId'] for r in results} == expected, 'R41_ALL_16_REQUIRED')
    ...
    frozen.require(all(type(r['gate']['capabilityMargins'][a]) in (int, float)
                       ...
                       for r in passing for a in axes), 'R41_NONFINITE_PASS_MARGIN')
...
def score_audited_artifact(root: Path, audit: dict, data, out: Path):
    ...
    """
    frozen.require(audit.get('status') == 'GEN2_ARTIFACT_AUDIT_PASS'
                   and audit.get('protocolSha256') == PROTOCOL_SHA256,
                   'R41_POST_RUN_INDEPENDENT_AUDIT_REQUIRED')
    frozen.require(not out.exists(), 'APPEND_ONLY_OUTPUT_EXISTS')
    protocol = load_protocol()
    for name, expected in protocol['baselineSourceHashes'].items():
        frozen.require(frozen.sha(frozen.ROOT / name) == expected, 'R41_BASELINE_SOURCE_DRIFT:' + name)
    ...
    for name in ledger_paths:
        frozen.require(audit['inputLedgerHashes'].get(name) == frozen.sha(root / name),
                       'R41_AUDITED_LEDGER_IDENTITY:' + name)
    ...
    for run in ('run-a', 'run-b'):
        ...
        for config in protocol['candidates']:
            ...
            frozen.require(all(row['candidateId'] == cid for row in rows), 'R41_LEDGER_CANDIDATE_ID')
    ...
    frozen.require(hashes_a == hashes_b and decisions[0] == decisions[1], 'R41_SCORE_AB_MISMATCH')
    receipt = {'schemaVersion': 'phase57-gen2-audited-scorecard-r41-v1',
               'protocolSha256': PROTOCOL_SHA256, 'selection': decisions[0],
               'candidateCount': 16, 'modelFitsPerformed': 0, 'policyReplaysPerformed': 0,
