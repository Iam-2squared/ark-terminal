...
export function scoreWindow(map,start,h,date){
 ...
 const first=map.get(start),last=map.get(start+h-5);
 if(!first||!last)return null;
 const valid=(b,m)=>{
  if(![b.open,b.high,b.low,b.close].every(x=>Number.isFinite(x)&&x>0)||b.low>Math.min(b.open,b.close)||b.high<Math.max(b.open,b.close)||minute(b.availableAtJst)!==m+5)throw Error('INVALID_BAR');
 };
 valid(first,start);valid(last,start+h-5);
 const gross=100*(last.close/first.open-1),rows=[];let complete=true;
 for(let m=start;m<start+h;m+=5){const b=map.get(m);if(!b){complete=false;continue;}valid(b,m);rows.push(b);}
 return {gross,MFE:complete?Math.max(0,...rows.map(b=>100*(b.high/first.open-1))):null,MAE:complete?Math.min(0,...rows.map(b=>100*(b.low/first.open-1))):null};
}
export function main(cache,context,out){
 ...
 try{for(const item of meta.files){
  ...
  console.log(JSON.stringify({status:'FROZEN_SCORE_ENDPOINTS_PROJECTED',sessionDate:date,eligibleRows:lines.length,providerRequests:0}));
