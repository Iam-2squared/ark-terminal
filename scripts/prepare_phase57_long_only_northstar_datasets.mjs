...
import {normalizeMinuteProvenance,auditRowSemantics} from '../predict/long-only/phase57-long-only-northstar-semantics.js';
...
const columns=[...new Set(['sessionDate','symbol','decisionTimeJst','partition','sourceGroup','segment','liquidityBucket','currentReturnPct','decisionVolatilityPct','legacyEligible',
  'legacyReferencePrice','legacyBucketAgeMin','legacySourceAgeMin','referenceAgeMin','referenceValid','referencePrice','referenceChanged','horizonMinutes','y30Bps','strict30Bps','strict30FreshBps',
  'rawMae30Pct','trueMae30Pct','legacySessionMfePct','legacySessionMaePct','legacyTrueSessionMaePct','futureMfePct','trueMaePct','futureMinuteCount',
  'timeTo1Min','timeTo2Min','timeTo3Min','timeTo5Min','futureOpportunity1','futureOpportunity2','futureOpportunity3','futureOpportunity5',
  'finalAdditionalPct','finalPreviousReturnPct','winner','savedV1Score',...features])];
...
for(const group of groups){
  ...
  try{
    for(const sessionDate of group.sessions){
      ...
      const intraday=normalizeAndAggregateMinuteRows(raw),provenance=normalizeMinuteProvenance(raw),barsMap=bySymbol(intraday.bars),dailyMap=new Map(dailyRows.map(r=>[symkey(r),r]));
      ...
      const rows=[];let legacyEligible=0,causalChecks=0;
      for(const feature of built.featureRows){
        ...
        const semantics=auditRowSemantics({feature,target,label,bars:barsMap.get(symkey(feature))??[],minutes:provenance.get(symkey(feature))??[],daily:dailyMap.get(symkey(feature))});
        const eligible=Number.isFinite(target?.y30Bps)?1:0;
        const row={...projected,sessionDate,symbol:feature.symbol,decisionTimeJst:feature.decisionTimeJst,partition,sourceGroup:group.name,segment:feature.segment,liquidityBucket:feature.liquidityBucket,
          decisionVolatilityPct:feature.decisionVolatilityPct,legacyEligible:eligible,...semantics,savedV1Score:scoreL2Candidate(feature,artifact)};
        if(semantics.referenceAgeMin<0||semantics.legacySourceAgeMin<0)throw new Error('future decision reference');
        rows.push(columns.map(c=>cell(row[c])).join('\t'));legacyEligible+=eligible;causalChecks++;
      }
      fs.writeSync(fd,rows.join('\n')+'\n');rowCount+=rows.length;
      sessionAudits.push({sessionDate,partition,sourceGroup:group.name,status:'AUDIT_INPUT_PREPARED',rawRows:raw.length,pageCount:pages.length,minuteAggregateSha256:aggregateSha,rows:rows.length,legacyEligible,causalChecks,...built.audit});
      console.log(JSON.stringify({status:'NORTHSTAR_SAVED_SESSION_PREPARED',sourceGroup:group.name,sessionDate,rows:rows.length,legacyEligible}));
    }
...
const manifest={schemaVersion:1,contractId:'NS-MEASUREMENT-1',contractCommit:'9acfd892ce8b86b8e70795e4f3950e3052529c67',sourceHead:'9d29ad7688677124ec584da346846c68fc6a40a0',files,columns,featureUniverse:features,sessionAudits,
  model:{name:'V1_SAVED_CD_REFIT',artifactSha256:artifact.artifactSha256,fitPartitions:['DEVELOPMENT_C','DEVELOPMENT_D'],trainingRows:artifact.heads[0].trainingRows,historicalCOnly:'UNAVAILABLE_NO_SAVED_WEIGHTS_OR_SCORES',v2:'UNAVAILABLE_NO_SAVED_WEIGHTS_OR_SCORES',notOriginal117bpsBenchmark:true},
...
console.log(JSON.stringify({status:'NORTHSTAR_PRIVATE_INPUT_READY',sessions:sessionAudits.filter(x=>x.status==='AUDIT_INPUT_PREPARED').length,rows:files.reduce((a,x)=>a+x.rows,0),fitCalls:0,providerRequests:0}));
