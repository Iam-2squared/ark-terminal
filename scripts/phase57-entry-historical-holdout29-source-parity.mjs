...
const PRECOMMIT_PATH = process.env.HOLDOUT_PARITY_PRECOMMIT_PATH ?? "predict/research/phase57-entry-historical-holdout29-source-parity-precommit.json";
const ADMISSION_PATH = process.env.ADMISSION_PATH ?? "artifacts/input/admission/admission.json";
const OUT_DIR = process.env.OUTPUT_DIR ?? "artifacts/phase57-entry-historical-holdout29-source-parity";
...
assert(precommit?.status === "HISTORICAL_HOLDOUT_SOURCE_PARITY_PRECOMMITTED_NO_OUTCOME_ACCESS", "holdout parity precommit not frozen");
assert(precommit?.purpose === "ENTRY_HISTORICAL_HOLDOUT_SOURCE_PARITY_ONLY_NO_FEATURES_NO_LABELS_NO_OUTCOMES", "scope mismatch");
assert(precommit?.holdoutMeasurementUnlocked === false, "precommit cannot unlock holdout measurement");
assert(precommit?.featureCalculationPerformed === false && precommit?.labelGenerationPerformed === false && precommit?.outcomeInspectionPerformed === false, "outcome-blind guards required");
assert(Array.isArray(precommit?.sessions) && precommit.sessions.length === 29 && precommit.count === 29, "expected exactly 29 sessions");
...
const output = {
  ...
  status: pass ? "HISTORICAL_HOLDOUT_SOURCE_PARITY_SHARD_PASS" : "HISTORICAL_HOLDOUT_SOURCE_PARITY_SHARD_FAIL",
