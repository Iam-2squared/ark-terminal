...
export function composeDirectionSpecializedFamilyExact(baselineRows=[],p2340Rows=[],p2345Rows=[]){
 ...
 for(const base of baselineRows){const src=base.direction==='DOWN'?a.get(key(base)):b.get(key(base));if(src&&Number.isFinite(src.riskScore)){out.push({...base,riskScore:src.riskScore,scoreSource:base.direction==='DOWN'?'P23_49_DOWN_P2340':'P23_49_UP_P2345'});if(base.direction==='DOWN')downCount++;else upCount++;}else{out.push({...base,scoreSource:'EXACT_P23_30_BASELINE'});preservedCount++;}}
 if(out.length!==baselineRows.length)throw Error('P23.49 baseline coverage changed');return{rows:out,upCount,downCount,preservedCount};
}
