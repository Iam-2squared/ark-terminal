...
for(const e of entered){
  ...
  tradeRows.push({eventId:e.eventId,sessionDate:e.sessionDate,symbol:e.symbol,direction:e.direction,entryTimestamp:e.decisionTimestamp,entryPrice,hybridRank:e.hybridRank??null,hybridScore:e.hybridScore??null,longProbability:e.longProbability,shortProbability:e.shortProbability,entryQuality:{h1:first,h3:l3,h6:l6,immediateAdverse:adverse,recoveredBy3Close,recoveredBy6Close,recoveredBy3Touch,recoveredBy6Touch,remainedLoserBy6},fixed12,sessionEnd,v3:{...v3,managementDecisions:undefined},v4:{...v4,managementDecisions:undefined},causal:{minimumObservedNeighborCount:Number.isFinite(minNeighbor)?minNeighbor:null,insufficientFallbackCount:fallbackCount,readiness}});
}
...
const classify=()=>{const integrity=managementIntegrity(readiness,tradeRows.length);if(integrity!=='SCORING_READY')return `20SESSION_DIAGNOSTIC_${integrity}`;const h3=entryQuality.horizons[3],entryGood=Number(h3?.meanNetBps)>0&&Number(h3?.positiveNetRate)>=0.5;const v34Weak=entryGood&&v3Metrics.averageTradePct<fixedMetrics.averageTradePct&&v4Metrics.averageTradePct<fixedMetrics.averageTradePct;return entryGood?(v34Weak?'20SESSION_DIAGNOSTIC_COMPLETE_ENTRY_GOOD_EXIT_V34_WEAK':'20SESSION_DIAGNOSTIC_COMPLETE_ENTRY_GOOD_EXIT_V34_USABLE'):'20SESSION_DIAGNOSTIC_COMPLETE_ENTRY_WEAK';};
const report={schemaVersion:1,status:classify(),role:cfg.role,providerLineage:'JQUANTS_EXISTING_15_PLUS_COMPLETION_5_SESSION_LEVEL_NO_MIXING',sessions:cfg.sessions.map(x=>x.sessionDate),sessionCount:cfg.sessions.length,entryQuality,arms:{fixed12:fixedMetrics,sessionEnd:sessionMetrics,v3:v3Metrics,v4:v4Metrics},pairedDelta:{v3MinusFixedAvgPct:v3Metrics.averageTradePct-fixedMetrics.averageTradePct,v4MinusFixedAvgPct:v4Metrics.averageTradePct-fixedMetrics.averageTradePct,v4MinusV3AvgPct:v4Metrics.averageTradePct-v3Metrics.averageTradePct},causal:{...causal,pitViolations:0},policy:{v3:P25_EXIT_V3_DUAL_GATE_POLICY_SHA256,v4:P25_EXIT_V4_POLICY_SHA256,analogSnapshot:PHASE57_P25_2K_POLICY.canonicalSnapshotSha256,candidate:cfg.lockedUpstream.candidateModelSha256},access:{diagnosticOutcomeSessions:cfg.sessions.length,protectedExternal:0,freshExternalOutsideDiagnostic:0,automaticExpansion:0},safety:cfg.safety};
