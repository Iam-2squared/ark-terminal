...
import {classifyGap} from './lib/phase57-offline-semantics.mjs';
...
export function historicalParity(root){
  ...
  for(const t of x.eligible){
    ...
    let decision=null;
    for(const row of rows){assert.equal(row.n,manager.n+1,'MANAGEMENT_GRID_GAP');const isExit=Date.parse(row.timestamp)===Date.parse(t.v4.exitTimestamp);const v4={action:isExit?'EXIT':'HOLD',timestamp:row.timestamp,...(isExit?{exitPrice:Number(t.v4.exitPrice),exitReason:t.v4.exitReason}:{})};const previousTimestamp=iso(manager.last);const result=manager.step({timestamp:row.timestamp,close:x.barCloseAt(t.symbol,row.timestamp),v4});if(result.gridGap)gaps.push({eventId:t.eventId,previousTimestamp,timestamp:row.timestamp,classification:classifyGap(previousTimestamp,row.timestamp,{savedFixture:true}),rootCause:"SOURCE_ABSENCE_VS_CAPTURE_LOSS_NOT_PROVEN"});steps.push(result);if(result.decision){decision=result.decision;break;}}
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  ...
  fs.writeFileSync(path.join(args[3],'gap-classification.json'),JSON.stringify({count:result.detail.gaps.length,trades:new Set(result.detail.gaps.map(x=>x.eventId)).size,counts:result.detail.gaps.reduce((a,x)=>(a[x.classification]=(a[x.classification]??0)+1,a),{}),gaps:result.detail.gaps},null,2)+'\n',{flag:'wx'});
