...
export function buildExitState({entryPrice,direction,bars}){
  ...
  const range=bars.slice(-12);const hi=Math.max(...range.map(b=>b.high)),lo=Math.min(...range.map(b=>b.low));
  const pos=hi>lo?(last.close-lo)/(hi-lo):0.5;
  const directionalRangePos=sign===1?pos:1-pos;
  return Object.freeze({
    currentReturnPct:currentReturn,
    bestReturnPct:bestReturn,
    givebackPctPoints:Math.max(0,bestReturn-currentReturn),
    atrPct:a/entryPrice*100,
...
function distance(a,b){
  ...
  return Math.sqrt(keys.reduce((s,k)=>{const scale=k==='bodyPressure'||k==='directionalRangePos'?1:Math.max(0.05,Math.abs(Number(a[k]))+Math.abs(Number(b[k])));const d=(Number(a[k])-Number(b[k]))/scale;return s+d*d;},0));
}
...
export function estimateExitEvidence(query,pool,policy=P23_18_EXIT_EVIDENCE_POLICY){
  ...
  const reversals=candidates.filter(x=>Number(x.nextDirectionalReturnPct)<0).length;
  const continuations=n-reversals;
  const alpha=policy.betaPriorAlpha+reversals,beta=policy.betaPriorBeta+continuations;
  const pReversal=alpha/(alpha+beta),pContinuation=1-pReversal;
  const se=Math.sqrt(pReversal*(1-pReversal)/(alpha+beta+1));
  const lowerReversal=clamp(pReversal-policy.confidenceZ*se,0,1);
  const expectedNextDirectionalReturnPct=mean(candidates.map(x=>x.nextDirectionalReturnPct));
  const exit=lowerReversal>0.5&&expectedNextDirectionalReturnPct<0;
  return Object.freeze({
    ready:true,neighborCount:n,pReversal,pContinuation,lowerReversal90:lowerReversal,
    expectedNextDirectionalReturnPct,decision:exit?'EXIT':'HOLD',
    reason:exit?'REVERSAL_PROBABILITY_CONFIRMED':'REVERSAL_NOT_CONFIRMED',
    evidenceSummary:{reversals,continuations,nearestDistance:candidates[0]?.d??null,furthestDistance:candidates.at(-1)?.d??null},
  });
}
