"""NEW LONG EXIT Profit Protection v5: lower CLOSE plus <=+2 after +3."""
...
CONTRACT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v5-contract-2026-09-18.md';OUT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v5-fast-fail'
def mod(n,p):
...
def sim(rows,fallback):
 ...
 for i,b in enumerate(rows):
  ...
  if b['c']<prev and b['c']<=2:
   ...
   trans.append([b['slot'],'LOWER_CLOSE_AT_OR_BELOW_PLUS2',peak,b['c']])
...
def build():
 v0=mod('pp_v0_for_v5',V0);old=v0.sim;v0.sim=sim
 try:ledger,s=v0.build()
 finally:v0.sim=old
 s['status']='NEW_LONG_EXIT_PROFIT_PROTECTION_V5_FAST_FAIL_PASS' if all(s['gates'].values()) else 'NEW_LONG_EXIT_PROFIT_PROTECTION_V5_FAST_FAIL_KILL'
 return ledger,s
def run(outdir):
