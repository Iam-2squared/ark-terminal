"""NEW LONG EXIT Profit Protection v1: first lower CLOSE after +3 without new high."""
from __future__ import annotations
import argparse,gzip,hashlib,importlib.util,json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
V0=ROOT/'scripts/phase57_new_long_exit_profit_protection_v0_fast_fail.py'
CONTRACT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v1-contract-2026-09-18.md'
OUT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v1-fast-fail'
def mod(n,p):
...
def sim(rows,fallback):
 protect=False;peak=0.;prev=None;trans=[]
 for i,b in enumerate(rows):
  ...
  if not protect:
   if peak>=3:protect=True;prev=b['c'];trans.append([b['slot'],'ENTER_PROTECT',peak,b['c']])
   continue
  if newpeak:prev=b['c'];trans.append([b['slot'],'NEW_HIGH',peak,b['c']]);continue
  if b['c']<prev:
   ...
   trans.append([b['slot'],'FIRST_GIVEBACK_CLOSE',peak,b['c']])
...
def build():
 v0=mod('pp_v0_for_v1',V0);old=v0.sim;v0.sim=sim
 try:ledger,s=v0.build()
 finally:v0.sim=old
 s['status']='NEW_LONG_EXIT_PROFIT_PROTECTION_V1_FAST_FAIL_PASS' if all(s['gates'].values()) else 'NEW_LONG_EXIT_PROFIT_PROTECTION_V1_FAST_FAIL_KILL'
 return ledger,s
def run(outdir):
