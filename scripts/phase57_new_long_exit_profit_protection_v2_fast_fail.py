"""NEW LONG EXIT Profit Protection v2: break PROTECT activation CLOSE."""
...
CONTRACT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v2-contract-2026-09-18.md';OUT=ROOT/'docs/evidence/phase57-new-long-exit-profit-protection-v2-fast-fail'
def mod(n,p):
...
def sim(rows,fallback):
 protect=False;peak=0.;activation=None;trans=[]
 for i,b in enumerate(rows):
  ...
  if not protect:
   if peak>=3:protect=True;activation=b['c'];trans.append([b['slot'],'ENTER_PROTECT',peak,activation])
   continue
  if newpeak:trans.append([b['slot'],'NEW_HIGH',peak,b['c']]);continue
  if b['c']<activation:
   ...
   trans.append([b['slot'],'ACTIVATION_CLOSE_BREAK',activation,b['c']])
...
def build():
 v0=mod('pp_v0_for_v2',V0);old=v0.sim;v0.sim=sim
 try:ledger,s=v0.build()
 finally:v0.sim=old
 s['status']='NEW_LONG_EXIT_PROFIT_PROTECTION_V2_FAST_FAIL_PASS' if all(s['gates'].values()) else 'NEW_LONG_EXIT_PROFIT_PROTECTION_V2_FAST_FAIL_KILL'
 return ledger,s
def run(outdir):
