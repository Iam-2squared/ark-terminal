...
const meanFinite=(items,key)=>{const values=items.map(x=>Number(x[key])).filter(Number.isFinite);return values.length?values.reduce((a,x)=>a+x,0)/values.length:null;};
const groupedStability=(trades,key)=>{const groups=new Map();for(const trade of trades){const group=String(trade[key]??'UNKNOWN');groups.set(group,(groups.get(group)??0)+(Number(trade.netPnlJpy)||0));}const values=[...groups.values()];return Object.freeze({groups:groups.size,profitableGroupPct:values.length?round(100*values.filter(x=>x>0).length/values.length):null,worstGroupNetJpy:values.length?round(Math.min(...values)):null});};
