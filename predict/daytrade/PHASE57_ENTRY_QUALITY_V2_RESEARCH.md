...
## Historical session and regime expansion audit

The next checkpoint is session diversity rather than a nominal 500-candidate count. Code inspection establishes that `PHASE57_P21_FROZEN_ENTRY` freezes a causal methodology, not one reusable fitted parameter vector. At every decision it intersects fully realized prior rows across horizons 1/3/6/12/24, runs the frozen inner walk-forward selection over horizon, feature family, model/config, and thresholds 0.55/0.60/0.65, then refits on prior-only rows. Its frozen universe is 7203.T, 6758.T, 9984.T, 8306.T, and 8035.T; the selection gates include 200 minimum training rows and 50 inner signals.

The current history pack ends on 2026-08-12, so `CURRENT_FROZEN_P21_BASELINE` can be replayed without that pack leaking future outcomes only from 2026-08-13 onward. An older decision requires a rolling history pack made solely from information realized before that decision. Such evidence is classified separately as `HISTORICALLY_RECONSTRUCTED_P21_METHODOLOGY`; it must never be described as the same current frozen baseline. Its earliest eligible date remains unclaimed until a real rolling reconstruction satisfies every common-horizon and inner-selection gate.

Official source review identifies J-Quants API v2 minute OHLC as the most practical pilot candidate: it documents one-minute OHLC, volume and trading value for TSE-listed equities over two years, supports an all-issues date query, and emits no row for a minute with no trade. It does not by itself prove whether `Time` is an interval-start or interval-end availability timestamp, the minute-price corporate-action basis, delisted/symbol-change completeness, or parity with the golden provider. Those questions require a small overlap pilot before any market-wide ingestion.

J-Quants API is individual private-use licensed. Raw data redistribution is prohibited, and cancellation or plan downgrade requires deletion of acquired data and reversible derivatives. Accordingly, provider raw data must not be committed to the repository without separately confirmed rights and access controls. No J-Quants credential or confirmed use right was available in this run, so the pilot was deliberately not executed and no new sessions were added.

J-Quants DataCube remains product-specific and its discovered TSE one-minute closing-price item has not been proven to provide the complete OHLCV market-wide input required by Current Dynamic5m. JPX FLEX Historical is exchange-origin and reaches back to 2011, but requires a contract, large PCAP/FLEX parsing and storage work, and materially higher fees. Neither is marked adoption-ready merely because it could increase sample count.

The two-year J-Quants retention is calendar-capable of the 30/50/100-session checkpoints, but that is not data readiness. Using the current 27.8125 candidates/session only as a mechanical capacity calculation gives approximately 834, 1,391, and 2,781 candidates respectively; these are not forecasts and may change with regimes, historical universe quality, missing bars, and causal prior eligibility.

Phase 0 therefore remains `NO_GO`: the dataset is still 445 candidates across 16 NORMAL_VOL sessions, no long-horizon provider has passed timestamp/OHLCV/universe overlap parity, the earliest causal historical-methodology date is not measured, and the precommitted untouched-OOS sessions remain future-only. No model was fitted, no threshold was tuned, and main/Lane Y were not changed.
