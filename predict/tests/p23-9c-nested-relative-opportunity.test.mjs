...
const ev=evaluateNestedRelativeOpportunity({trainingExamples:train,frozenTestExamples:frozen,config:cfg});assert.equal(ev.integrity.frozenOuterOutcomesUsedForSelection,false);assert.equal(ev.integrity.calibrationPrecedesFrozenWindow,true);assert.equal(ev.edgeClaimAllowed,false);assert.equal(ev.executionAllowed,false);
if(ev.status==='NESTED_RELATIVE_OPPORTUNITY_EVALUATED'){assert.equal(ev.integrity.sameSessionTrainingForbidden,true);for(const p of ev.predictions)assert.equal(p.outerOutcomeUsedForSelection,false);}else{assert.equal(ev.calibration.integrity.sameSessionTrainingForbidden,true);}
