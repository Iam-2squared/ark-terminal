"""Frozen-contract LONG v2.1 anchor refinement: one weighted Ridge adverse-magnitude model.
...
CONTRACT_SHA = '82c17234b482118919b092df4af56c0e6a60d792de5ce3a551a509bcc3d293c7'
MODEL_ID = 'MSH_ENTRY_LONG_V2_1_V1_ANCHOR_D30_RIDGE_V1'
FEATURE_ORDER = ['directionalMomentum3Pct',
...
def extract_inputs(source):
    ...
    for name in FEATURE_ORDER[:2]:
        ...
        if value['status'] == 'AVAILABLE':
            ...
            raw.append(None)
            missing.append(1)
    valid = (finite(source['decisionPrice']) and source['decisionPrice'] > 0)
...
def validate_envelope(row):
    ...
    if len(row['raw']) != 2 or len(row['missing']) != 2:
        raise IntegrityError('FEATURE_ORDER_DIMENSION')
    for j,bit in enumerate(row['missing']):
    ...
    if row['valid'] and not (finite(row['decisionPrice']) and row['decisionPrice'] > 0):
...
def fit(rows, labels):
    ...
    medians = [weighted_median([r[j] for r in raw],weights,ids) for j in (0,1)]
    for j in range(2):
        if sum(r[j] is not None for r in raw) < 2:
            raise FitError('FIT_FAILED_OBSERVED_SUPPORT')
    imputed = np.asarray([[r[j] if r[j] is not None else medians[j] for j in range(2)] for r in raw],dtype=np.float64)
    ...
    A = X.T@(weights[:,None]*X)+np.diag([0.,1.,1.,1.,1.])
    rhs = X.T@(weights*y)
    try:
    ...
    artifact = {'schemaVersion':1,'modelId':MODEL_ID,'contractSHA':CONTRACT_SHA,
        ...
        'training':{'candidateRows':len(ordered),'eligibleRows':len(eligible),'excludedLabelRows':len(excluded),
            ...
            'weights':list(zip(ids,weights.tolist())),
            'symbolSessionCount':len({(r['symbol'],r['sessionDate']) for r in eligible}),
            'largestSymbolWeightShare':max(sum(float(w) for r,w in zip(eligible,weights) if r['symbol']==s) for s in {r['symbol'] for r in eligible}),
            'largestSymbolSessionWeightShare':max(sum(float(w) for r,w in zip(eligible,weights) if (r['symbol'],r['sessionDate'])==sd) for sd in {(r['symbol'],r['sessionDate']) for r in eligible}),
            'bothMissingCount':int(np.sum(np.all(masks==1,axis=1))),
            'missingCounts':[int(np.sum(masks[:,j])) for j in range(2)],
            'observedCounts':[sum(r[j] is not None for r in raw) for j in range(2)],
...
def validate_artifact(artifact):
    ...
    if (a['contractSHA'] != CONTRACT_SHA or a['modelId'] != MODEL_ID or a['featureOrder'] != FEATURE_ORDER
        or a['lambda'] != 1. or len(a['coefficients']) != 4):
        raise IntegrityError('MODEL_CONTRACT_MISMATCH')
    for key,n in [('medians',2),('means',2),('stds',2),('coefficients',4)]:
    ...
    for row in sorted(rows,key=sort_key):
        ...
        if reason:
            output.append(base|{'status':reason,'rawPrediction':None,'predictedD30':None,
                'rawInputs':row['raw'],'missing':row['missing'],'transformedInputs':None,'contributions':None})
            continue
        r = row['raw']
        imputed = [r[j] if r[j] is not None else artifact['medians'][j] for j in range(2)]
        ...
        output.append(base|{'status':'SCORED','rawPrediction':raw,'predictedD30':max(0.,raw),
            'rawInputs':row['raw'],'missing':row['missing'],'transformedInputs':values,
            'contributions':[float(v*b) for v,b in zip(values,artifact['coefficients'])]})
    return output



def decide(rows, opportunity, predictions, threshold):
    """Apply Risk only to immutable v1 shadow ENTER anchors; never retry a veto.

    No labels/path/EXIT input. The upstream shadow state has already consumed the
    first qualified anchor. Its later SKIPs cannot be recovered by Risk scores.
    """
    ...
    expected = {eid for eid in ids if opportunity[eid]['state'] == 'ENTER'}
    by_score = {p['eventId']:p for p in predictions}
    if len(by_score) != len(predictions) or set(by_score) != expected:
        raise IntegrityError('RISK_PREDICTIONS_MUST_MATCH_EXACT_V1_ANCHORS')
    consumed = set()
    output = []
    for row in sorted(rows,key=sort_key):
        validate_envelope(row)
        eid = row['eventId']
        op = opportunity[eid]
        key = row['symbol'],row['sessionDate']
        base = {k:row[k] for k in ['eventId','symbol','sessionDate','decisionTimestamp']}
        base.update({'threshold':threshold,'opportunityState':op['state'],
                     'opportunityReason':op['reason'],'opportunityScore':op['expectedClass']})
        if op['state'] != 'ENTER':
            if op['state'] != 'SKIP_THIS_DECISION':
                raise IntegrityError('UNKNOWN_V1_STATE')
            output.append(base|{'state':'SKIP_THIS_DECISION','reason':'NOT_OPPORTUNITY_ANCHOR',
                                'status':'NOT_OPPORTUNITY_ANCHOR','rawPrediction':None,'predictedD30':None})
            continue
        if key in consumed or not finite(op['expectedClass']) or op['expectedClass'] < 2.:
            raise IntegrityError('INVALID_SHADOW_ANCHOR')
        consumed.add(key)  # before Risk; rejected/unsupported anchors still consume.
        p = by_score[eid]
        if any(p[k] != base[k] for k in ['symbol','sessionDate','decisionTimestamp']):
            raise IntegrityError('PREDICTION_IDENTITY_MISMATCH')
        ...
        if p['status'] == 'SCORED':
            ...
            if p['predictedD30'] <= threshold:
                state,reason = 'ENTER','RISK_ACCEPT_AT_V1_ANCHOR'
            else:
                reason = 'DIRECT_RISK_VETO'
        output.append(p|base|{'state':state,'reason':reason,'shadowAnchorConsumed':True})
    return output
