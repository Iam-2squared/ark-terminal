...
assert.equal(out.methodology.managementDecisionEvidencePersisted,true);
assert.equal(out.methodology.arbitraryHoldExtension,false);
assert.equal(out.methodology.postOutcomeRuleSelection,false);
assert.equal(out.methodology.performanceConclusionAllowed,false);
assert.ok(out.pairs[0].dynamic.decisionEvidence.length>0);
assert.ok(out.pairs[0].dynamic.decisionEvidence.every(decision=>typeof decision.reason==='string'&&decision.reason.length>0));
assert.equal(out.pairs[0].dynamic.evidenceAudit.reasonsPresent,true);
assert.equal(out.evidenceAudit.everyDecisionHasReason,true);
assert.equal(out.evidenceAudit.everyHealthDecisionHasCoreEvidence,true);
assert.equal(out.evidenceAudit.arbitraryHoldExtensionUsed,false);
assert.equal(out.evidenceAudit.resultBasedRetuningAllowed,false);
assert.ok(out.evidenceAudit.decisionBasis.includes('market_structure'));
assert.ok(out.evidenceAudit.decisionBasis.includes('VWAP'));
assert.ok(out.evidenceAudit.decisionBasis.includes('ATR_hard_stop'));
...
console.log('P25 dynamic management parallel evidence regression test passed');
