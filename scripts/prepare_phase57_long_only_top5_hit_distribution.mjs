// Evaluator-only reconstruction for ELIGIBILITY-TOP5-HIT-DISTRIBUTION-1.
...
const columns=[...new Set(['sessionDate','symbol','decisionTimeJst','partition','sourceGroup','segment','liquidityBucket','currentReturnPct','decisionVolatilityPct',
  ...
  'highOpportunity1','highOpportunity2','highOpportunity3','highOpportunity5','closeOpportunity1','closeOpportunity2','closeOpportunity3','closeOpportunity5',
  'auctionCloseOpportunity1','auctionCloseOpportunity2','auctionCloseOpportunity3','auctionCloseOpportunity5',
  'timeToHigh1Min','timeToHigh2Min','timeToHigh3Min','timeToHigh5Min','timeToClose1Min','timeToClose2Min','timeToClose3Min','timeToClose5Min',
  ...
  try{
    for(const sessionDate of group.sessions){
      ...
      for(const feature of built.featureRows){
        ...
        const bars=barsMap.get(symkey(feature))??[],auctions=auctionMap.get(symkey(feature))??[];
        const semantics=measureCorrectedRow({feature,target,label,bars,minutes:provenance.get(symkey(feature))??[],terminalAuctions:auctions,daily:dailyMap.get(symkey(feature))});
        ...
        const row={...projected,sessionDate,symbol:feature.symbol,decisionTimeJst:feature.decisionTimeJst,partition,sourceGroup:group.name,segment:feature.segment,liquidityBucket:feature.liquidityBucket,
          decisionVolatilityPct:feature.decisionVolatilityPct,...semantics,...onePct,savedV1Score:scoreL2Candidate(feature,artifact)};
      ...
      sessionAudits.push({sessionDate,partition,sourceGroup:group.name,status:'TOP5_HIT_INPUT_PREPARED',rawRows:raw.length,pageCount:pages.length,minuteAggregateSha256:aggregateSha,rows:rows.length,freshDecisionPrice,corrected30Evaluable,causalChecks,...built.audit});
      console.log(JSON.stringify({status:'TOP5_HIT_SAVED_SESSION_PREPARED',sourceGroup:group.name,sessionDate,rows:rows.length,freshDecisionPrice,corrected30Evaluable}));
    }
...
const manifest={schemaVersion:1,contractId:'CORRECTED-MEASUREMENT-1',contractCommit:'20356553ddece7309a6a00654c3b1e276d1a3466',sourceHead:'962b96fd74964081d23dcdfc37d87fc00a98200f',files,columns,featureUniverse:features,sessionAudits,
  ...
  evaluatorOnlyAddition:'FUTURE_1PCT_HIGH_AND_CLOSE_FROM_SAVED_MINUTE_PATH'};
fs.writeFileSync(path.join(outputDir,'dataset-manifest.json'),JSON.stringify(manifest,null,2)+'\n',{flag:'wx',mode:0o600});
console.log(JSON.stringify({status:'TOP5_HIT_PRIVATE_INPUT_READY',sessions:sessionAudits.filter(x=>x.status==='TOP5_HIT_INPUT_PREPARED').length,rows:files.reduce((a,x)=>a+x.rows,0),fitCalls:0,providerRequests:0}));
