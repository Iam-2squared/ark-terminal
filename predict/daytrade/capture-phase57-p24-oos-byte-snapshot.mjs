    ...
    for(const host of [1,2]){
      ...
      for(let a=1;a<=4;a++){
        try{
          ...
          clearTimeout(t);
          if(!res.ok)throw Error(`${symbol} Yahoo HTTP ${res.status}`);
          responses[url]={status:res.status,statusText:res.statusText,headers:Object.fromEntries(res.headers.entries()),body:await res.text()};
          last=null;break;
...
const snapshot={phase:'57.p24.9-oos-byte-snapshot',purpose:'pre-performance canonical OOS provider capture; no model or trade metrics computed during capture',dataEndIso,symbols,responseCount:Object.keys(responses).length,responses,methodology:{performanceObservedBeforeFreeze:false,parameterSweep:false,postHocSymbolFiltering:false,entryRetuning:false,freshHoldoutConsumed:false},safety:{executionAllowed:false,brokerWriteAllowed:false,excelOrderWriteAllowed:false,rssOrderFunctionAllowed:false,liveTradingAllowed:false,paperTradingAllowed:false,automaticPromotionAllowed:false,productionUpdateAllowed:false,transmitted:false,freshHoldoutConsumed:false}};
