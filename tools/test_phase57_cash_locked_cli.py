...
class CashLockedCliTests(unittest.TestCase):
    def test_named_fields_keep_numeric_types_and_separate_symbols(self):
        ...
        self.assertEqual(value["intent"]["symbol"], "7203.T")
    ...
    def test_blank_symbol_rejected_not_shifted_or_defaulted(self):
        for symbol in (None, "", " "):
            ...
            with self.subTest(symbol=symbol), self.assertRaisesRegex(ValueError, "ORDER_SYMBOL_REQUIRED"):
    ...
    def test_quantity_is_never_parsed_from_another_field(self):
        for qty in ("123A", "100", True, 100.5, 0, -100):
            ...
            with self.subTest(qty=qty), self.assertRaisesRegex(ValueError, "ORDER_QUANTITY"):
                validate_request(value)
    ...
    def test_bom_unicode_paths_no_timestamp_rewrite(self):
        with tempfile.TemporaryDirectory(prefix="ark spaces ") as root:
            root = Path(root)
            snapshot_path = root / "口座 snapshot.json"
            source = {"capturedAt": "2026-09-15T10:00:00+09:00", "buyingPower": 17}
            snapshot_path.write_text(json.dumps(source), encoding="utf-8-sig")
            request_path = root / "request.json"
            request_path.write_text(json.dumps(request(snapshot_path)), encoding="utf-8-sig")
            output = root / "result.json"
            original_bytes = snapshot_path.read_bytes()
            with patch("phase57_cash_locked_cli._evaluate", return_value={"status": "BLOCKED", "stage": "G6"}) as evaluate:
                self.assertEqual(main(["--request", str(request_path), "--output", str(output)]), 0)
            self.assertEqual(evaluate.call_args.args[0], source)
            self.assertEqual(snapshot_path.read_bytes(), original_bytes)
            result = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(result["status"], "BLOCKED")
            self.assertTrue(result["inspectionOnly"])

    def test_invalid_request_exits_nonzero_without_ready_output(self):
        with tempfile.TemporaryDirectory() as root:
            root = Path(root); path = root / "request.json"; output = root / "result.json"
            value = request(); value["intent"]["quantity"] = "123A"
            path.write_text(json.dumps(value), encoding="utf-8")
            with redirect_stderr(io.StringIO()), patch("phase57_cash_locked_cli._evaluate") as evaluate:
                self.assertEqual(main(["--request", str(path), "--output", str(output)]), 2)
    ...
    def test_invalid_pipeline_response_does_not_produce_ready(self):
        for bad in ({}, {"status": "LOCKED_READY"}, {"status": "OK", "stage": "X"}):
            with tempfile.TemporaryDirectory() as root:
                root = Path(root); account = root / "snapshot.json"; path = root / "request.json"; output = root / "result.json"
                account.write_text("{}", encoding="utf-8")
                path.write_text(json.dumps(request(account)), encoding="utf-8")
                with redirect_stderr(io.StringIO()), patch("phase57_cash_locked_cli._evaluate", return_value=bad):
                    self.assertEqual(main(["--request", str(path), "--output", str(output)]), 2)
                self.assertFalse(output.exists())

    def test_source_file_cannot_be_used_as_output(self):
        with tempfile.TemporaryDirectory() as root:
            root = Path(root); account = root / "snapshot.json"; path = root / "request.json"
            account.write_text("{}", encoding="utf-8")
            path.write_text(json.dumps(request(account)), encoding="utf-8")
            with redirect_stderr(io.StringIO()):
                self.assertEqual(main(["--request", str(path), "--output", str(account)]), 2)
