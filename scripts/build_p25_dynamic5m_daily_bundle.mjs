...
const inputDir=arg('--input-dir'),sessionDate=arg('--session-date'),output=arg('--output',`tmp/p25-dynamic5m-daily-${sessionDate}.json`);
...
const normalize=(rows=[],observedAt)=>rows.map(r=>({symbol:String(r?.symbol??'').trim().toUpperCase(),sector:String(r?.sector??'UNKNOWN'),currentPrice:Number(r?.currentPrice),sourceScannedAt:r?.sourceScannedAt??observedAt,opportunityScore:Number(r?.opportunityScore),turnoverYen:Number(r?.turnoverYen),...(Number.isFinite(Number(r?.v2Score))?{v2Score:Number(r.v2Score)}:{}),...(r?.components?{components:r.components}:{})})).filter(r=>r.symbol);
...
for(const file of walk(inputDir)){
  ...
  const b=bucket(observedAt),key=new Date(b).toISOString(),selected=normalize(x.selected,observedAt),selectedV2=normalize(x.selectedV2??[],observedAt);
  if(selected.length<20)continue;
  const candidate={bucket:key,observedAt,sourceFile:file,inputSymbols:Number(x.inputSymbols),selected,selectedV2,priorV2PointCount:Number(x.priorV2PointCount??0),methodology:x.methodology,policy:x.policy,dynamic5mV2:x.dynamic5mV2??null,safety:x.safety};
...
const expected=[];for(const [start,end] of [['09:00','11:30'],['12:30','15:30']]){let t=Date.parse(`${sessionDate}T${start}:00+09:00`),e=Date.parse(`${sessionDate}T${end}:00+09:00`);for(;t<=e;t+=5*60_000)expected.push(new Date(t).toISOString());}
const actual=new Set(points.map(x=>x.bucket)),missingExpectedBuckets=expected.filter(x=>!actual.has(x));
const symbolsV1=[...new Set(points.flatMap(p=>p.selected.map(x=>x.symbol)))].sort(),symbolsV2=[...new Set(points.flatMap(p=>p.selectedV2.map(x=>x.symbol)))].sort(),symbols=[...new Set([...symbolsV1,...symbolsV2])].sort();
const barsBySymbol={},failures=[];let cursor=0;
...
const completeBars=failures.length===0&&Object.keys(barsBySymbol).length===symbols.length,hasV2EveryPoint=points.every(p=>p.selectedV2.length>=15);
...
const payload={schemaVersion:2,phase:'57.p25.dynamic5m-daily-bundle',status:completeBars?'P25_DYNAMIC5M_DAILY_BUNDLE_READY':'BLOCKED_P25_DYNAMIC5M_DAILY_BUNDLE',sessionDate,createdAt:new Date().toISOString(),pointCount:points.length,expectedPointCount:expected.length,missingExpectedBucketCount:missingExpectedBuckets.length,missingExpectedBuckets,completeFiveMinuteCoverage:missingExpectedBuckets.length===0,hasDynamic5mV2EveryPoint:hasV2EveryPoint,selectedSymbolUnionCount:symbolsV1.length,selectedSymbolUnion:symbolsV1,selectedV2SymbolUnionCount:symbolsV2.length,selectedV2SymbolUnion:symbolsV2,allRequiredBarSymbolUnionCount:symbols.length,points,barsReady:completeBars,barFailureCount:failures.length,barFailures:failures,sessionBarsBySymbol:barsBySymbol,methodology:{selectionMembershipCapturedPointInTime:true,dynamic5mV2MembershipCapturedPointInTime:hasV2EveryPoint,scannerPriceRetainedAtObservedAt:true,postCloseBarsMayBeUsedOnlyAsPrefixAtRecordedSelectionCutoff:true,missingIntradayBucketsNeverBackfilledOrFabricated:true,fullDayValidityRequiresCompleteFiveMinuteCoverage:true,entryThresholdRelaxed:false,resultBasedRetuning:false,freshHoldoutConsumed:false},safety};
fs.mkdirSync(path.dirname(output),{recursive:true});fs.writeFileSync(output,JSON.stringify(payload,null,2)+'\n');
console.log(JSON.stringify({status:payload.status,sessionDate,pointCount:points.length,expectedPointCount:expected.length,missingExpectedBucketCount:missingExpectedBuckets.length,selectedSymbolUnionCount:symbolsV1.length,selectedV2SymbolUnionCount:symbolsV2.length,hasDynamic5mV2EveryPoint:hasV2EveryPoint,barsReady:completeBars,sha256:sha(fs.readFileSync(output))},null,2));
