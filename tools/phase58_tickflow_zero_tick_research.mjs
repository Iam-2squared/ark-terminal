...
function flowFor(r){const order=tickOrderOf(r);if(order!=='ASC'&&order!=='DESC')throw new Error(`row missing valid tickOrder: ${order||'EMPTY'}`);return buildZeroTickFlow(r?.ticks??[],{inputOrder:order});}
function diag(rows){
...
const stats={}; let cfSum=0,cfN=0,cfMin=null,cfMax=null,continuation=0,ticks=0,normalizedRows=0;
for(let i=0;i<raw.length;i++){
 const built=flowFor(raw[i]); const flow=built.features; if(built.integrity.causalOrderNormalized)normalizedRows++;
 const cf=Number(flow.classifiedTickFraction);
