...
## Bounded tail dash slot (2026-09-14)

The user observed valid 15:00/15:05/15:10 rows followed by an all-seven-cell dash row and a blank row. The earlier DATE_PARSE at row 148 is consistent with that row having been the moving dash slot during capture, but its original capture-time contents were not independently observed here.

Before date/time parsing, the RAW validator recognizes seven nonblank dash-only cells using the shared Phase58 placeholder helper. It requires at least one previously validated numeric bar, and enters a tail-only state: every subsequent nonblank row must also be all-seven-cell dash. Any later valid, timestamped unpopulated, partial or unknown row fails closed. Leading dash slots without valid bars fail. Blank spacers remain ignored. This stricter tail rule never fabricates timestamps or bars, nor proves finalization.

`skippedTailPlaceholderRowCount` and `skippedTailPlaceholderRows` retain physical Excel row and reason `RSSCHART_FUTURE_DASH_SLOT`. Tail slots do not count toward valid bars, latest-N or rawLastExcelRow. Full raw scans remain preserved. Two successive synthetic workbook snapshots cover row148 dash becoming 15:10 numeric and row149 becoming dash; capture/normalization/diagnostic has nonempty bars, latest parity PASS and events=[]. Existing Phase58 regressions remain unchanged.

Use the existing one-command 90-second setup diagnostic. On the user's Windows machine, confirm nonempty bars, latestRawWindowMetadata, normalizationParity=PASS, equal rawLatestSessionSourceTime/normalizedLatestSourceTime, and recorded tail evidence. Actual patched source capture is pending; no BAR_FINALIZATION_SEMANTICS_PASS or REALTIME_SHADOW_PASS. Frozen Main, #583, OOS, freshness and order paths are unchanged.
